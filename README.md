...
- 可选轻量背景修复 adapter：不保持原布局时会对原元素区域生成本地背景补丁；保持原布局时自动跳过背景修复。
...
- 尚未接入语义目标检测、inpainting 或大模型风格生成。
- 已接入可选轻量检测、SAM2.1 tiny 分割、RapidOCR 轻量 OCR 和轻量本地风格迁移；其他 UI 选项如 `YOLO26`、`ControlNet + IPAdapter` 当前仍只是记录选择。
...
- Optional lightweight background-repair adapter: when layout preservation is disabled, it creates local background patches for original element regions; when layout is preserved, background repair is skipped.
...
- Optional lightweight detection, SAM2.1 tiny segmentation, RapidOCR lightweight OCR, and lightweight local style transfer are connected; other UI choices such as `YOLO26` and `ControlNet + IPAdapter` are currently recorded only.
