...
- 尚未接入真实目标检测、inpainting 或大模型风格生成。
- 已接入可选 SAM2.1 tiny 分割、RapidOCR 轻量 OCR 和轻量本地风格迁移；其他 UI 选项如 `YOLO26`、`ControlNet + IPAdapter` 当前仍只是记录选择。
...
- No real object detection, inpainting, or large-model style generation yet.
- Optional SAM2.1 tiny segmentation, RapidOCR lightweight OCR, and lightweight local style transfer are connected; other UI choices such as `YOLO26` and `ControlNet + IPAdapter` are currently recorded only.
