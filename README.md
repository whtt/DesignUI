...
- 尚未接入真实目标检测、抠图、inpainting 或风格迁移。
- 已接入可选 SAM2.1 tiny 分割和 RapidOCR 轻量 OCR；其他 UI 选项如 `YOLO26`、`ControlNet + IPAdapter` 当前仍只是记录选择。
...
- No real object detection, cutout, inpainting, or style transfer yet.
- Optional SAM2.1 tiny segmentation and RapidOCR lightweight OCR are connected; other UI choices such as `YOLO26` and `ControlNet + IPAdapter` are currently recorded only.
